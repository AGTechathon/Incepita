...
public class QuestionPaperController {
    ...
    public ResponseEntity<ArrayList<QuestionPaper>> getAllQuestionPapers(@PathVariable String username){
        ArrayList<QuestionPaper> papersList =questionPaperService.getAllQuestionPapers(username);
        for(QuestionPaper paper : papersList)
            Collections.sort(paper.getQuestionList());
        return ResponseEntity.ok(papersList);
    }
...
//    Get the paper by username, paperid, version
    ...
    public ResponseEntity<QuestionPaper> getQuestionPaper(@PathVariable String username, @PathVariable String questionPaperId, @PathVariable(required = false) Integer targetVersion) {
        ...
        QuestionPaper dbPaper = questionPaperService.getQuestionPaper(username, questionPaperId);
        ...
        dbPaper.setQuestionList(questionService.getVersionOrFallback(questionPaperId, targetVersion));
        return ResponseEntity.ok(dbPaper);
    }
